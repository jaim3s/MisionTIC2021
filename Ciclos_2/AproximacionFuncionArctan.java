...
/**
 * Aproximacion de funcion arctan con series de Maclaurin alrededor de 0.
 */
class AproximacionFuncionArctan{
    ...
    public static double arctan(double x, int n){
        double cnt = 0;
        for (int i = 0; i <= n; i++){
            cnt += (potencia(-1, i) * potencia(x, 2*i+1))/(2*i + 1);
        }
        return cnt;
    }
