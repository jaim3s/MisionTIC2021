...
/**
 * Aproximacion de funcion seno con series de Maclaurin alrededor de 0.
 */
class AproximacionFuncionSeno{
    ...
    public static double seno(double x, int n){
        double cnt = 0;
        for (int i = 0; i <= n; i++){
            cnt += (potencia(-1, i) * potencia(x, 2*i+1))/(factorial(2*i+1));
        }
        return cnt;
    }
